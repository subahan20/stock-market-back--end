...
import { buildAiDigestHtml, buildAiDigestText, sendMail } from './email.service.js';
...
export async function sendAiInsightsEmail({ user, name, email }) {
  ...
  const rows = await loadAiDigestRows();
  const subject = `Stock Insights · AI digest (${rows.length} symbol${rows.length === 1 ? '' : 's'})`;
  const html = buildAiDigestHtml({ recipientName: cleanedName, rows });
  const text = buildAiDigestText({ recipientName: cleanedName, rows });
