...
/**
 * Wraps the raw HTML generated by Groq into the professional email layout.
 */
export function buildGroqEmailTemplate({ recipientName, contentHtml, generatedAt }) {
  const ts = generatedAt || new Date().toISOString();
  const tsLabel = new Date(ts).toLocaleString('en-IN', {
    dateStyle: 'medium',
    timeStyle: 'short',
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1" />
  <title>Stock Insights · Groq AI Digest</title>
</head>
<body style="margin:0;padding:0;background:#020617;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',sans-serif;color:#e2e8f0;">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#020617;padding:32px 16px;">
    <tr>
      <td align="center">
        <table role="presentation" cellpadding="0" cellspacing="0" width="640" style="max-width:640px;border-collapse:collapse;background:#0b1220;border:1px solid #1e293b;border-radius:14px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:0 24px 16px 24px; font-size:14px; line-height:1.6; color:#cbd5e1;">
              ${contentHtml}
            </td>
          </tr>
          <tr>
            <td style="padding:8px 24px 24px 24px; border-top: 1px solid #1e293b;">
              <p style="margin:12px 0 0 0;font-size:10px;color:#475569;line-height:1.6;">
                This digest was generated by AI based on live market data. It is for informational purposes only and does not constitute financial advice.
              </p>
            </td>
          </tr>
        </table>
